...
import { getDatabase, get, onValue, ref, set } from 'https://www.gstatic.com/firebasejs/12.18.0/firebase-database.js';
...
(() => {
  ...
  const LEGACY_FIREBASE_STATE_PATH = 'apps/massageCredits/sharedState';
  const MAX_HISTORY = 1000;
  const DEFAULT_PIN = '01123456';
  const USER_NAMES = Object.freeze({ pai: 'Pai', mae: 'Mãe' });
  const ACTIVE_USER_ID = Object.prototype.hasOwnProperty.call(USER_NAMES, localStorage.getItem(ACTIVE_USER_KEY))
  ...
  function applyRemoteState(remoteState) {
    ...
    appState = normaliseState(remoteState);
      ...
      if (!firebaseStateListenerBound) {
        ...
        onValue(
          ...
          async (snapshot) => {
            ...
            if (!snapshot.exists()) {
              if (!legacyMigrationAttempted && !hasStoredV2State) {
                legacyMigrationAttempted = true;
                try {
                  const legacySnapshot = await get(ref(database, LEGACY_FIREBASE_STATE_PATH));
                  if (legacySnapshot.exists()) {
                    appState = normaliseState(legacySnapshot.val());
                    state = appState.users[ACTIVE_USER_ID];
                    saveLocalState();
                    recoverSession();
                    renderAll();
                  ...
                  console.warn('Não foi possível migrar os dados da versão anterior:', migrationError);
                }
              }
  ...
  function getLevel(targetState = state, userId = ACTIVE_USER_ID) {
    const vipAutomatic = targetState.stats.totalMassageUses >= 15 || targetState.stats.totalMinutesUsed >= 180;
    if (targetState.stats.vipManual || vipAutomatic) {
      return {
        name: `VIP ${getUserName(userId)}`,
        className: 'vip',
        caption: 'Nível especial ativo. Experiência privada no patamar máximo.'
      };
    }
    if (targetState.credits > 150) {
      return {
        name: 'Ouro',
        className: 'gold',
  ...
  function applyAutomaticRewards(targetState = state, userId = ACTIVE_USER_ID) {
    ...
    const vipAutomatic = targetState.stats.totalMassageUses >= 15 || targetState.stats.totalMinutesUsed >= 180;
    if (vipAutomatic && !targetState.stats.rewards.vipUnlockedLogged) {
      targetState.stats.rewards.vipUnlockedLogged = true;
      addHistory(
        'reward',
        `Nível VIP ${getUserName(userId)} desbloqueado automaticamente`,
        null,
        { reward: 'vip' },
        targetState
      );
  ...
  function renderAdmin() {
    ...
    const level = getLevel(targetState, adminTargetUserId);
    ...
    elements.adminVipTitle.textContent = `VIP ${targetName}`;
    ...
    elements.vipToggle.checked = Boolean(targetState.stats.vipManual);
  ...
  function handleCreditsSubmit(event) {
    ...
    const rewarded = applyAutomaticRewards(targetState, adminTargetUserId);
  ...
  function handleVipToggle() {
    const targetState = getAdminState();
    const targetName = getUserName(adminTargetUserId);
    targetState.stats.vipManual = elements.vipToggle.checked;
    addHistory(
      'admin',
      `Nível VIP ${targetName} ${targetState.stats.vipManual ? 'ativado' : 'desativado'} manualmente`,
      null,
      { admin: true },
      targetState
    );
    saveState({ changedState: targetState });
    renderAll();
    showToast(`VIP ${targetName} ${targetState.stats.vipManual ? 'ativado' : 'desativado'}.`);
  }
  ...
  function registerServiceWorker() {
    ...
    navigator.serviceWorker
      .register('./service-worker.js?v=3.5.0', { updateViaCache: 'none' })
  ...
  function cacheElements() {
    Object.assign(elements, {
      ...
      profileSwitchButton: $('#profileSwitchButton'),
  ...
  function bindEvents() {
    ...
    elements.profileSwitchButton.addEventListener('click', changeLoginUser);
  ...
  function init() {
    ...
    appState = loadState();
    state = appState.users[ACTIVE_USER_ID];
    elements.activeUserName.textContent = getUserName(ACTIVE_USER_ID);
    saveLocalState();
    bindEvents();
    recoverSession();
    renderAll();
