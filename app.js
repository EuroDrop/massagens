...
(() => {
  ...
  const STORAGE_KEY = 'massageCreditsApp_v2';
  const LEGACY_STORAGE_KEY = 'massageCreditsApp_v1';
  const PENDING_SYNC_KEY = 'massageCreditsPendingSync_v2';
  const DEVICE_ID_KEY = 'massageCreditsDeviceId_v1';
  const ACTIVE_USER_KEY = 'massageCreditsUser_v1';
  const FIREBASE_STATE_PATH = 'apps/massageCredits/v2/sharedState';
  const MAX_HISTORY = 1000;
  const DEFAULT_PIN = '01123456';
  const USER_NAMES = Object.freeze({ pai: 'Pai', mae: 'Mãe' });
  const LOGIN_CONTEXT = localStorage.getItem(ACTIVE_USER_KEY);
  const IS_STANDALONE_ADMIN = LOGIN_CONTEXT === 'admin';
  const ACTIVE_USER_ID = Object.prototype.hasOwnProperty.call(USER_NAMES, LOGIN_CONTEXT)
    ? LOGIN_CONTEXT
    : 'pai';
  ...
  let appState;
  let state;
  let adminUnlocked = IS_STANDALONE_ADMIN;
  let adminTargetUserId = ACTIVE_USER_ID;
  let pendingAdminNavigation = false;
  let currentView = IS_STANDALONE_ADMIN ? 'admin' : 'home';
  ...
  let sessionAuthorisationMode = 'minute';
  let pendingCouponInstanceId = null;
  ...
  function createDefaultUserState(userId = 'pai') {
      ...
      history: [
        {
          ...
          date: createdAt,
          category: 'credits',
          description: `+80 créditos adicionados — Saldo inicial de ${USER_NAMES[userId] || 'utilizador'}`,
          creditsDelta: 80,
          meta: { admin: true }
        }
      ],
  ...
  function createDefaultState() {
    ...
    return {
      ...
      users: {
        pai: createDefaultUserState('pai'),
        mae: createDefaultUserState('mae')
      },
      createdAt,
      updatedAt: createdAt
    };
  ...
  function normaliseUserState(saved, userId = 'pai') {
    const base = createDefaultUserState(userId);
    ...
    const merged = {
      ...
      stats: {
        totalMinutesUsed: Math.max(0, Math.floor(Number(saved.stats?.totalMinutesUsed) || 0)),
        totalMassageSessions: Math.max(0, Math.floor(Number(saved.stats?.totalMassageSessions) || 0)),
        totalMassageUses: Math.max(0, Math.floor(Number(saved.stats?.totalMassageUses) || 0)),
        totalCouponsPurchased: Math.max(0, Math.floor(Number(saved.stats?.totalCouponsPurchased) || 0)),
        totalCouponsUsed: Math.max(0, Math.floor(Number(saved.stats?.totalCouponsUsed) || 0)),
        totalCreditsAdded: Math.max(0, Math.floor(Number(saved.stats?.totalCreditsAdded) || 0)),
        totalCreditsRemoved: Math.max(0, Math.floor(Number(saved.stats?.totalCreditsRemoved) || 0)),
        totalCreditsSpent: Math.max(0, Math.floor(Number(saved.stats?.totalCreditsSpent) || 0)),
        rewards: {
          massage5Milestones: Math.max(0, Math.floor(Number(saved.stats?.rewards?.massage5Milestones) || 0)),
          minutes60Milestones: Math.max(0, Math.floor(Number(saved.stats?.rewards?.minutes60Milestones) || 0)),
          reached100: Boolean(saved.stats?.rewards?.reached100)
        }
      },
    ...
    merged.shopCoupons = merged.shopCoupons.map((coupon) => ({
      ...
      durationMinutes: Math.max(1, Math.floor(Number(coupon.durationMinutes) || 1)),
      active: coupon.active !== false
    }));

    merged.ownedCoupons = merged.ownedCoupons.map((coupon) => {
      const durationMinutes = Math.max(1, Math.floor(Number(coupon.durationMinutes) || 1));
      const defaultRemainingSeconds = durationMinutes * 60;
      const remainingSeconds = Math.max(
        0,
        Math.min(defaultRemainingSeconds, Math.floor(Number(coupon.remainingSeconds ?? defaultRemainingSeconds)))
      );
      const isUsed = coupon.status === 'used' || remainingSeconds === 0;
      ...
      return {
        ...
        description: String(coupon.description || ''),
        durationMinutes,
        remainingSeconds: isUsed ? 0 : remainingSeconds,
        purchasedAt: coupon.purchasedAt || isoNow(),
        usedAt: isUsed ? (coupon.usedAt || isoNow()) : null,
  ...
  function normaliseState(saved) {
    ...
    if (saved.users && typeof saved.users === 'object') {
      return {
        ...
        users: {
          pai: normaliseUserState(saved.users.pai, 'pai'),
          mae: normaliseUserState(saved.users.mae, 'mae')
        },
        createdAt: saved.createdAt || base.createdAt,
        updatedAt: saved.updatedAt || base.updatedAt
      };
    ...
    // Migração da versão anterior: todos os dados existentes pertenciam ao Pai.
    return {
      version: 2,
      adminPin: /^\d{4,8}$/.test(String(saved.adminPin || '')) ? String(saved.adminPin) : DEFAULT_PIN,
      users: {
        pai: normaliseUserState(saved, 'pai'),
        mae: base.users.mae
      },
      createdAt: saved.createdAt || base.createdAt,
      updatedAt: saved.updatedAt || base.updatedAt
  ...
  function loadState() {
    try {
      ...
      return legacyRaw ? normaliseState(JSON.parse(legacyRaw)) : createDefaultState();
    } catch (error) {
  ...
  function queueCloudSave() {
    ...
    cloudSaveChain = cloudSaveChain
      ...
      .then(() => {
        ...
        if (String(appState.updatedAt || '') === payloadUpdatedAt) {
  ...
  function saveState({ syncCloud = true, changedState = state } = {}) {
    const updatedAt = isoNow();
    if (changedState) changedState.updatedAt = updatedAt;
  ...
  function applyRemoteState(remoteState) {
    const canonicalState = normaliseState(remoteState);
    const needsCanonicalWrite = JSON.stringify(canonicalState) !== JSON.stringify(remoteState);

    applyingRemoteState = true;
    appState = canonicalState;
    state = appState.users[ACTIVE_USER_ID];
    saveLocalState();
      ...
      if (!firebaseStateListenerBound) {
        ...
        onValue(
          ...
          (snapshot) => {
            ...
            const localUpdatedAt = Date.parse(appState.updatedAt || '') || 0;
            ...
            if (JSON.stringify(remoteState) !== JSON.stringify(appState)) {
  ...
  function getUserName(userId) {
    return USER_NAMES[userId] || 'Utilizador';
  }

  function getAdminState() {
    return appState.users[adminTargetUserId] || state;
  }

  function addHistory(category, description, creditsDelta = null, meta = {}, targetState = state) {
    targetState.history.unshift({
      id: uid('history'),
      date: isoNow(),
      category,
      description,
    ...
    targetState.history = targetState.history.slice(0, MAX_HISTORY);
  }
  ...
  function applyAutomaticRewards(targetState = state) {
    let rewardGiven = false;

    const usesMilestone = Math.floor(targetState.stats.totalMassageUses / 5);
    while (targetState.stats.rewards.massage5Milestones < usesMilestone) {
      targetState.stats.rewards.massage5Milestones += 1;
      targetState.credits += 10;
      targetState.stats.totalCreditsAdded += 10;
      addHistory(
        'reward',
    ...
    const minutesMilestone = Math.floor(targetState.stats.totalMinutesUsed / 60);
    while (targetState.stats.rewards.minutes60Milestones < minutesMilestone) {
      targetState.stats.rewards.minutes60Milestones += 1;
      const rewardCoupon = {
      ...
      targetState.ownedCoupons.unshift(rewardCoupon);
      addHistory(
        'reward',
    ...
    if (targetState.credits >= 100 && !targetState.stats.rewards.reached100) {
      targetState.stats.rewards.reached100 = true;
      targetState.ownedCoupons.unshift({
        instanceId: uid('owned'),
  ...
  function ownedCouponCard(coupon) {
    const remainingSeconds = Math.max(0, Number(coupon.remainingSeconds) || 0);
    const remainingText = remainingSeconds % 60 === 0
      ? plural(remainingSeconds / 60, 'minuto restante', 'minutos restantes')
      : `${formatDuration(remainingSeconds)} restantes`;
    const buttonLabel = remainingSeconds < coupon.durationMinutes * 60 ? 'Continuar cupão' : 'Usar cupão';
    ...
    return `
      <article class="coupon-card ${coupon.isReward ? 'reward' : ''}">
        <div class="coupon-top">
          ...
          <span class="coupon-price">${escapeHTML(remainingText)}</span>
        </div>
        <p>${escapeHTML(coupon.description)}</p>
        <div class="coupon-meta">
          <small>Comprado em ${escapeHTML(formatDate(coupon.purchasedAt, { day: '2-digit', month: 'short', year: 'numeric' }))}</small>
          <button class="primary-button small" type="button" data-use-coupon="${escapeHTML(coupon.instanceId)}">${buttonLabel}</button>
        </div>
      </article>
    `;
  }
  ...
  function renderMinute() {
    ...
    const couponSession = session?.active && session.mode === 'coupon';

    elements.timerCard.classList.toggle('coupon-session', Boolean(couponSession));
    elements.minuteEyebrow.textContent = couponSession ? 'Cupão em utilização' : 'Sessão flexível';
    elements.minuteTitle.textContent = couponSession ? session.couponName : 'Massagem ao Minuto';
    elements.minuteIntro.textContent = couponSession
      ? 'O cronómetro desconta diretamente do tempo ainda disponível neste cupão.'
      : 'Controla o tempo, pausa quando precisares e termina com um resumo claro.';
    elements.minutePriceDisplay.textContent = couponSession ? '⌁' : String(session?.pricePerMinute || state.minutePrice);
    elements.pricePillLabel.textContent = couponSession ? 'Cupão' : 'crédito/min';
    elements.timerHelpText.textContent = couponSession
      ? 'O tempo do cupão é descontado ao segundo. Se terminares antes do fim, o tempo restante continua disponível em “Os meus cupões”.'
      : 'É cobrado o preço definido por cada minuto iniciado. A sessão termina automaticamente quando já não existir saldo para iniciar o minuto seguinte.';

    if (session?.active) {
      elements.timerCard.classList.add('active-session');
      elements.timerDisplay.textContent = formatDuration(couponSession ? session.couponRemainingSeconds : session.totalSeconds);
      elements.sessionPrimaryLabel.textContent = couponSession ? 'Tempo utilizado' : 'Créditos usados';
      elements.sessionSecondaryLabel.textContent = couponSession ? 'Tempo restante' : 'Saldo restante';
      elements.sessionCredits.textContent = couponSession ? formatDuration(session.totalSeconds) : String(session.chargedCredits);
      elements.sessionBalance.textContent = couponSession ? formatDuration(session.couponRemainingSeconds) : String(state.credits);
      ...
      elements.timerProgress.style.width = couponSession
        ? `${Math.max(0, Math.min(100, (session.couponRemainingSeconds / session.couponStartingRemainingSeconds) * 100))}%`
        : `${((session.totalSeconds % 60) / 60) * 100}%`;
      ...
      elements.timerCard.classList.remove('active-session', 'coupon-session');
      elements.minuteEyebrow.textContent = 'Sessão flexível';
      elements.minuteTitle.textContent = 'Massagem ao Minuto';
      elements.minuteIntro.textContent = 'Controla o tempo, pausa quando precisares e termina com um resumo claro.';
      elements.minutePriceDisplay.textContent = String(state.minutePrice);
      elements.pricePillLabel.textContent = 'crédito/min';
      elements.timerHelpText.textContent = 'É cobrado o preço definido por cada minuto iniciado. A sessão termina automaticamente quando já não existir saldo para iniciar o minuto seguinte.';
      elements.timerDisplay.textContent = '00:00';
      elements.sessionPrimaryLabel.textContent = 'Créditos usados';
      elements.sessionSecondaryLabel.textContent = 'Saldo restante';
    ...
    if (state.lastSessionSummary) {
      const couponSummary = state.lastSessionSummary.mode === 'coupon';
      ...
      elements.summaryCreditsLabel.textContent = couponSummary ? 'Cupão' : 'Créditos';
      elements.summaryBalanceLabel.textContent = couponSummary ? 'Tempo restante' : 'Saldo';
      elements.summaryCredits.textContent = couponSummary ? state.lastSessionSummary.couponName : `${state.lastSessionSummary.credits} cr.`;
      elements.summaryBalance.textContent = couponSummary
        ? formatDuration(state.lastSessionSummary.couponRemainingSeconds)
        : `${state.lastSessionSummary.balanceAfter} cr.`;
    } else {
  ...
  function renderAdmin() {
    ...
    const targetState = getAdminState();
    const targetName = getUserName(adminTargetUserId);
    elements.adminUserSelect.value = adminTargetUserId;
    elements.adminUserHint.textContent = `As alterações abaixo serão aplicadas exclusivamente ao perfil ${targetName}.`;
    elements.adminSummary.innerHTML = `
      <div><span>Utilizador</span><strong>${escapeHTML(targetName)}</strong></div>
      <div><span>Saldo atual</span><strong>${targetState.credits} cr.</strong></div>
      <div><span>Utilizações</span><strong>${targetState.stats.totalMassageUses}</strong></div>
      <div><span>Minutos usados</span><strong>${targetState.stats.totalMinutesUsed}</strong></div>
      <div><span>Cupões comprados</span><strong>${targetState.stats.totalCouponsPurchased}</strong></div>
      <div><span>Sessões ao minuto</span><strong>${targetState.stats.totalMassageSessions}</strong></div>
    `;

    elements.minutePriceInput.value = String(targetState.minutePrice);
    updateAdminDiagnostics();

    elements.adminCouponList.innerHTML = targetState.shopCoupons.length
      ? targetState.shopCoupons.map((coupon) => `
          <article class="admin-coupon-item ${coupon.active ? '' : 'inactive'}">
            <div>
              <strong>${escapeHTML(coupon.name)}</strong>
              <small>${coupon.price} créditos · ${coupon.durationMinutes} min · ${coupon.active ? 'Ativo' : 'Inativo'}</small>
            </div>
  ...
  function navigateTo(view, bypassAdminCheck = false) {
    if (IS_STANDALONE_ADMIN && view !== 'admin') return;
  ...
  async function buyCoupon(couponId) {
    ...
    state.ownedCoupons.unshift({
      ...
      remainingSeconds: coupon.durationMinutes * 60,
      purchasedAt: isoNow(),
      usedAt: null,
      status: 'active',
      isReward: false
  ...
  function useCoupon(instanceId) {
    const coupon = state.ownedCoupons.find((item) => item.instanceId === instanceId && item.status === 'active');
    if (!coupon) return;

    if (state.activeSession?.active) {
      if (state.activeSession.mode === 'coupon' && state.activeSession.couponInstanceId === instanceId) {
        navigateTo('minute');
      } else {
        showToast('Termina primeiro a sessão que está em curso.');
      }
      return;
    }

    sessionAuthorisationMode = 'coupon';
    pendingCouponInstanceId = instanceId;
    elements.sessionPinTitle.textContent = 'Autorizar utilização do cupão';
    elements.sessionPinDescription.textContent = `Introduz o PIN para iniciar “${coupon.name}”. O tempo será descontado do cupão.`;
    elements.sessionPinSubmitButton.textContent = 'Autorizar e usar cupão';
  ...
  function requestMinuteSessionStart() {
    ...
    sessionAuthorisationMode = 'minute';
    pendingCouponInstanceId = null;
    elements.sessionPinTitle.textContent = 'PIN do administrador';
    elements.sessionPinDescription.textContent = 'Apenas o administrador pode iniciar a massagem ao minuto.';
    elements.sessionPinSubmitButton.textContent = 'Autorizar e iniciar';
    elements.sessionPinInput.value = '';
    elements.sessionPinError.textContent = '';
    openDialog(elements.sessionPinModal);
    window.setTimeout(() => elements.sessionPinInput.focus(), 120);
  ...
  function authoriseSessionStart(event) {
    ...
    if (sessionAuthorisationMode === 'coupon') {
      startCouponSession(pendingCouponInstanceId);
    } else {
      startMinuteSession();
    }
    pendingCouponInstanceId = null;
  ...
  function startCouponSession(instanceId) {
    if (state.activeSession?.active) return;
    const coupon = state.ownedCoupons.find((item) => item.instanceId === instanceId && item.status === 'active');
    if (!coupon) {
      showToast('Este cupão já não está disponível.');
      return;
    }

    const remainingSeconds = Math.max(0, Math.floor(Number(coupon.remainingSeconds) || 0));
    if (remainingSeconds <= 0) {
      coupon.status = 'used';
      coupon.usedAt = coupon.usedAt || isoNow();
      saveState();
      renderAll();
      showToast('Este cupão já não tem tempo disponível.');
      return;
    }
    ...
    state.activeSession = {
      ...
      couponStartingRemainingSeconds: remainingSeconds,
      couponRemainingSeconds: remainingSeconds,
      startedAt: isoNow(),
    ...
    navigateTo('minute');
    renderAll();
    showToast('Cupão autorizado. O tempo começou a ser descontado.');
  }
  ...
  function syncSessionTime() {
    ...
    if (session.mode === 'coupon') {
      const remainingBefore = Math.max(0, Math.floor(Number(session.couponRemainingSeconds) || 0));
      const consumedSeconds = Math.min(elapsedSeconds, remainingBefore);
      session.totalSeconds += consumedSeconds;
      session.couponRemainingSeconds = Math.max(0, remainingBefore - consumedSeconds);
      session.lastTimestamp = previous + elapsedSeconds * 1000;

      const coupon = state.ownedCoupons.find((item) => item.instanceId === session.couponInstanceId);
      if (coupon) coupon.remainingSeconds = session.couponRemainingSeconds;

      if (session.couponRemainingSeconds === 0) {
        finishMinuteSession('Tempo do cupão concluído', true);
        return true;
      }

      if (session.totalSeconds - lastPersistedSecond >= 5) {
  ...
  function finishMinuteSession(reason = 'Terminada pelo utilizador', automatic = false) {
    ...
    const isCouponSession = finalSession.mode === 'coupon';
    const usedMinutes = finalSession.totalSeconds > 0 ? Math.ceil(finalSession.totalSeconds / 60) : 0;

    if (finalSession.totalSeconds > 0) {
    ...
    if (isCouponSession) {
      const coupon = state.ownedCoupons.find((item) => item.instanceId === finalSession.couponInstanceId);
      const remainingSeconds = Math.max(0, Math.floor(Number(finalSession.couponRemainingSeconds) || 0));
      const couponCompleted = remainingSeconds === 0;

      if (coupon) {
        coupon.remainingSeconds = remainingSeconds;
        if (couponCompleted && coupon.status !== 'used') {
          coupon.status = 'used';
      ...
      addHistory(
        'coupons',
        couponCompleted
          ? `Cupão ${finalSession.couponName} concluído — ${formatDuration(finalSession.totalSeconds)} utilizados`
          : `Sessão com o cupão ${finalSession.couponName} terminada — ${formatDuration(finalSession.totalSeconds)} utilizados — ${formatDuration(remainingSeconds)} restantes`,
        null,
        {
          coupon: true,
          massageUse: finalSession.totalSeconds > 0,
          sessionId: finalSession.id,
          couponInstanceId: finalSession.couponInstanceId,
      ...
      state.lastSessionSummary = {
        ...
        minutes: usedMinutes,
        couponName: finalSession.couponName,
        couponRemainingSeconds: remainingSeconds,
        reason
      };
    } else {
    ...
    saveState();
    renderAll();
    const baseMessage = isCouponSession
      ? (finalSession.couponRemainingSeconds === 0 ? 'Cupão concluído.' : 'Sessão terminada. O tempo restante continua disponível no cupão.')
      : (automatic ? 'Sessão terminada automaticamente por falta de saldo.' : 'Sessão terminada e registada.');
  ...
  function handleCreditsSubmit(event) {
    ...
    if (operation === 'remove' && requestedAmount > targetState.credits) {
    ...
    targetState.credits += delta;

    if (delta > 0) targetState.stats.totalCreditsAdded += delta;
    else targetState.stats.totalCreditsRemoved += Math.abs(delta);
    ...
    const rewarded = applyAutomaticRewards(targetState);
    saveState({ changedState: targetState });
    event.target.reset();
    renderAll();
    showToast(rewarded ? `Movimento aplicado a ${targetName}. Nova recompensa desbloqueada.` : `Créditos de ${targetName} atualizados.`);
  }

  async function resetBalance() {
    const targetState = getAdminState();
    const targetName = getUserName(adminTargetUserId);
    if (targetState.activeSession?.active) {
      showToast('Termina primeiro a sessão de massagem em curso.');
      return;
    }
    const accepted = await askConfirmation({
      title: 'Repor saldo',
      message: `O saldo de ${targetName}, atualmente com ${targetState.credits} créditos, será reposto para zero.`,
    ...
    const removed = targetState.credits;
    targetState.credits = 0;
    if (removed > 0) targetState.stats.totalCreditsRemoved += removed;
    addHistory('admin', `Saldo reposto para zero pelo admin — ${removed} créditos removidos`, removed ? -removed : 0, { admin: true }, targetState);
    saveState({ changedState: targetState });
    renderAll();
    showToast(`Saldo de ${targetName} reposto para zero.`);
  }
  ...
  function handleMinutePriceSubmit(event) {
    ...
    const previous = targetState.minutePrice;
    targetState.minutePrice = value;
    addHistory('admin', `Preço da massagem ao minuto alterado — ${previous} para ${value} créditos`, null, { admin: true }, targetState);
    saveState({ changedState: targetState });
    renderAll();
    showToast(targetState.activeSession?.active ? `Preço de ${targetName} guardado para as próximas sessões.` : `Preço por minuto de ${targetName} atualizado.`);
  }

  function openCouponEditor(couponId = null) {
    const targetState = getAdminState();
    const targetName = getUserName(adminTargetUserId);
    const coupon = couponId ? targetState.shopCoupons.find((item) => item.id === couponId) : null;
    elements.couponModalTitle.textContent = `${coupon ? 'Editar' : 'Novo'} cupão — ${targetName}`;
    ...
    elements.couponDurationInput.value = coupon?.durationMinutes ?? 1;
  ...
  function handleCouponEditorSubmit(event) {
    ...
    if (!name || !description || !Number.isFinite(price) || price < 1 || !Number.isFinite(durationMinutes) || durationMinutes < 1) {
      showToast('Preenche todos os dados do cupão corretamente.');
      return;
    }

    if (editId) {
      const coupon = targetState.shopCoupons.find((item) => item.id === editId);
      if (!coupon) return;
      coupon.name = name;
      ...
      addHistory('admin', `Cupão editado — ${name}`, null, { admin: true, coupon: true }, targetState);
    } else {
      targetState.shopCoupons.push({
        id: uid('coupon'),
        name,
        description,
        price,
      ...
      addHistory('admin', `Novo cupão criado — ${name}`, null, { admin: true, coupon: true }, targetState);
    }

    saveState({ changedState: targetState });
    closeDialog(elements.couponModal);
    renderAll();
    showToast(editId ? `Cupão de ${targetName} atualizado.` : `Novo cupão criado para ${targetName}.`);
  }

  async function deleteCoupon(couponId) {
    const targetState = getAdminState();
    const targetName = getUserName(adminTargetUserId);
    const coupon = targetState.shopCoupons.find((item) => item.id === couponId);
    if (!coupon) return;

    const accepted = await askConfirmation({
      title: 'Apagar cupão',
      message: `O cupão “${coupon.name}” será removido da loja de ${targetName}. Os cupões já comprados não serão afetados.`,
      acceptLabel: 'Apagar',
      danger: true,
      icon: '×'
    });
    ...
    targetState.shopCoupons = targetState.shopCoupons.filter((item) => item.id !== couponId);
    addHistory('admin', `Cupão apagado da loja — ${coupon.name}`, null, { admin: true, coupon: true }, targetState);
    saveState({ changedState: targetState });
    renderAll();
    showToast(`Cupão removido da loja de ${targetName}.`);
  }
  ...
  function handlePinChange(event) {
    ...
    addHistory('admin', 'PIN de administração alterado', null, { admin: true }, getAdminState());
    saveState({ changedState: getAdminState() });
    event.target.reset();
  ...
  async function clearHistory() {
    ...
    const accepted = await askConfirmation({
      ...
      message: `Todos os registos de atividade de ${targetName} serão apagados em todos os dispositivos. As estatísticas e o saldo serão mantidos.`,
    ...
    saveState({ changedState: targetState });
    renderAll();
    showToast(`Histórico de ${targetName} limpo.`);
  }
  ...
  function handleAdminUserChange() {
    const requestedUserId = elements.adminUserSelect.value;
    if (!Object.prototype.hasOwnProperty.call(USER_NAMES, requestedUserId)) return;
    adminTargetUserId = requestedUserId;
    renderAdmin();
    showToast(`Painel preparado para gerir ${getUserName(adminTargetUserId)}.`);
  }
  ...
  function registerServiceWorker() {
    ...
    navigator.serviceWorker
      .register('./service-worker.js?v=3.6.0', { updateViaCache: 'none' })
  ...
  function cacheElements() {
    Object.assign(elements, {
      ...
      minuteEyebrow: $('#minuteEyebrow'),
      minuteTitle: $('#minuteTitle'),
      minuteIntro: $('#minuteIntro'),
      timerCard: $('#timerCard'),
      timerStatus: $('#timerStatus'),
      minutePriceDisplay: $('#minutePriceDisplay'),
      pricePillLabel: $('#pricePillLabel'),
      timerDisplay: $('#timerDisplay'),
      timerProgress: $('#timerProgress'),
      sessionPrimaryLabel: $('#sessionPrimaryLabel'),
      ...
      summaryCreditsLabel: $('#summaryCreditsLabel'),
      ...
      sessionPinTitle: $('#sessionPinTitle'),
      sessionPinDescription: $('#sessionPinDescription'),
      sessionPinSubmitButton: $('#sessionPinSubmitButton'),
  ...
  function bindEvents() {
    ...
    elements.adminUserSelect.addEventListener('change', handleAdminUserChange);
    ...
    elements.sessionPinForm.addEventListener('submit', authoriseSessionStart);
  ...
  function recoverSession() {
    const session = state.activeSession;
    if (!session?.active) return;
    session.mode = session.mode === 'coupon' ? 'coupon' : 'minute';
    session.totalSeconds = Math.max(0, Math.floor(Number(session.totalSeconds) || 0));

    if (session.mode === 'coupon') {
      const coupon = state.ownedCoupons.find((item) => item.instanceId === session.couponInstanceId);
      const fallbackRemaining = coupon?.remainingSeconds ?? 0;
      session.couponRemainingSeconds = Math.max(0, Math.floor(Number(session.couponRemainingSeconds ?? fallbackRemaining) || 0));
      session.couponStartingRemainingSeconds = Math.max(
        session.couponRemainingSeconds,
        Math.floor(Number(session.couponStartingRemainingSeconds) || session.couponRemainingSeconds)
      );
      session.couponName = String(session.couponName || coupon?.name || 'Cupão');
  ...
  function enforceMobileViewport() {
    // Evita pinch-zoom e double-tap zoom na interface da app.
    ...
    // Bloqueio de orientação quando a plataforma o permite (PWA/standalone).
    ...
    tryLockOrientation();
    window.addEventListener('focus', tryLockOrientation);
    document.addEventListener('visibilitychange', () => {
      if (document.visibilityState === 'visible') tryLockOrientation();
    });
  }
  ...
  function init() {
    ...
    appState = loadState();
    state = appState.users[ACTIVE_USER_ID];
    document.body.classList.toggle('standalone-admin', IS_STANDALONE_ADMIN);
    elements.activeUserName.textContent = IS_STANDALONE_ADMIN ? 'Admin' : getUserName(ACTIVE_USER_ID);
    if (IS_STANDALONE_ADMIN) elements.lockAdminButton.textContent = 'Sair';
    saveLocalState();
    bindEvents();
    recoverSession();
    renderAll();
    if (IS_STANDALONE_ADMIN) navigateTo('admin', true);
